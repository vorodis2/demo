...
import { GeomParse } from './geomParse/GeomParse.js';
import { ConvexGeometry  } from './geomParse/ConvexGeometry.js';
...
export class PhBlok {
    constructor(par, fun) {
        ...
        //кидаем модель, по ней строим физику из кусков
        this.addPhisikC3d = function(c3d, textPar, _fun,p) {            
            if(_fun!=undefined){
                this.geomParse=new GeomParse(this, c3d, textPar,_fun,p)
                return
            }

            this.geomParse=new GeomParse(this, c3d, textPar, function(s,p){
                if(s=="creatArray"){
                    for (var i = 0; i < p.length; i++) { 
                        ...
                        let shepe = self.getShape(o,undefined,self.arrShepes.length)//this.getShape(this.obj.arrShepes[i],undefined,i)

                        self.arrShepes.push(shepe);
                        self.hron.addShapes(shepe);
                    }                    
                }               
            },p)
        ...
        this.getMat = function() {
            let mm=this.par.mat1
           /* mm= new THREE.MeshBasicMaterial({
        ...
        this.isConvexHull = function(o) {
            
            if(o && o.aStr && o.aStr[1] && o.aStr[1].indexOf('convexHull')!=1  ){
                let a=o.aStr[1].split('|');
                if(a[0]=='convexHull'){
                    ...
                    for (var i = 1; i < a.length; i++) {
                        ...
                        oo.tip="convexHull"
                        oo.a=aa
                        let shepe = this.getShape(oo)//this.getShape(this.obj.arrShepes[i],undefined,i)
        ...
        var _shape
        this.boolHHH=false
        let pz=new THREE.Vector3()
        this.getShape = function(o, boolNot,ii, posPlus) {
            ...
            let m = this.par.mat0
            if (this.obj.mass != 0) m = this.getMat()//this.par.mat1
            ...
            if (o.tip == "convexHull") {
                ...
                for (var i = 0; i < o.a.length; i++) {
                    a.push(new OIMO.Vec3(o.a[i].x,o.a[i].y,o.a[i].z))  

                    ppp.add(o.a[i])                  
                }
            
                //var sc = new OIMO.ShapeConfig();
                shapeConfig.geometry = new OIMO.ConvexHullGeometry(a);
                            
                const geometry = new ConvexGeometry(o.a);
                ...
                dd=0
                for (var i = 0; i < o.a.length; i++) {
                    d=this.hron.metodPBH.getDistance3d(ppp, o.a[i])
                    if(d>dd)dd=d
            ...
            if(shapeConfig==null)return null
            ...
            shepe.vectPlus = new THREE.Vector3() //смещение
            if(posPlus)shepe.vectPlus.set(posPlus.x,posPlus.y,posPlus.z )   
        ...
        this.shapeForstPosition = function(shepe) {
            ...
            _position = new OIMO.Vec3(shepe.vectPlus.x, shepe.vectPlus.y, shepe.vectPlus.z)
            _rotation = new OIMO.Quat(0, 0, 0, 1)

            if (shepe.oInfo.p) {
                _position.x = shepe.oInfo.p.x+shepe.vectPlus.x;
                _position.y = shepe.oInfo.p.y+shepe.vectPlus.y;
                _position.z = shepe.oInfo.p.z+shepe.vectPlus.z;
            }
        ...
        this.start = function(o) {
            ...
            if (this.obj.massBody !== undefined) this.obj.mass = this.obj.massBody   
            ...
            if (this.obj.arrShepes) {
                ...
                for (var i = 0; i < this.obj.arrShepes.length; i++) {
                    if(this.isConvexHull(this.obj.arrShepes[i])==true)continue


                    let shepe = this.getShape(this.obj.arrShepes[i],undefined,i)//this.getShape(this.obj.arrShepes[i],undefined,i)
       ...
       /* this.creatShape = function(o,boolNot,ii, posPlus) {
            let shepe = this.getShape(o,boolNot,ii,posPlus)
